		...
		Schema: spec.Schema{
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"labels": {
						SchemaProps: spec.SchemaProps{
							Description: "Map of string keys and values that can be used to organize and categorize (scope and select) this related image, for instance, by the product features it belongs to. Keys and values follow the Kubernetes label syntax and constraints, see https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#syntax-and-character-set. The semantics of the labels is defined by their consumer, which typically picks related images with Kubernetes label selectors. The first consumer is `oc-mirror`, see https://github.com/openshift/oc-mirror/blob/main/README.md for how it selects the related images to mirror. Labeling a related image makes it optional for mirroring: `oc-mirror` mirrors it only if the entry for its package in the ImageSetConfiguration carries a selector that matches the labels. If that package entry has no matching selector, the image is skipped. Related images without labels are always mirrored. Note that `operator-sdk generate bundle` may not recognize these labels and drop them from the generated CSV. It is the operator author's responsibility to make sure the labels end up in the bundle's CSV, restoring them manually if needed.",
							Type:        []string{"object"},
							AdditionalProperties: &spec.SchemaOrBool{
								Allows: true,
								Schema: &spec.Schema{
